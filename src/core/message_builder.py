...
class YearStats:
    ...
    def _calculate_bar(self, length: int, fill_char: str, empty_char: str) -> tuple[str, str]:
    ...
    def render_bar(self, length: int = 10, style: Literal["line", "block", "solid", "empty"] = "block") -> str:
    ...
    def render_wecom_html(self, length: int = 10, fill_char: str = "─", empty_char: str = "─") -> str:
        ...
        num_color = "info" if self.progress < 30 else "warning" if self.progress < 85 else "comment"
...
class MessageContextBuilder:
    ...
    @staticmethod
    def build(today_status: dict[str, Any], current_time: datetime.datetime, settings: Settings) -> dict:
        ...
        context = {
            ...
            "year_progress_status": YearStats(CalendarUtil.get_year_progress(current_time)),
            "workweek_status": MessageContextBuilder.get_workweek_status(today),
            "festivals": CalendarUtil.get_upcoming_festivals(today),
