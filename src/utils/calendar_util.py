...
class CalendarUtil:
    ...
    @staticmethod
    def get_friday_of_week(date_value: datetime.date | datetime.datetime | None = None) -> datetime.date:
    ...
    def get_nth_weekday(year: int, month: int, nth: int, weekday: int) -> datetime.date:
        ...
        month_days = [
            day for week in cal.monthdays2calendar(year, month)
            for day, wd in week if day != 0 and wd == weekday
        ]
    ...
    @staticmethod
    def get_lunar_date(date_value: datetime.date | datetime.datetime | str | None = None) -> LunarDate:
        target = CalendarUtil.normalize_date(date_value)
    ...
    def get_upcoming_festivals(
        ...
        """
        Get a list of upcoming festivals from settings.FESTIVALS after the given date.
        ...
        target_size = len([
            f for f in settings.FESTIVALS if include_disabled or f.get('enabled', True)
        ])
        ...
        for year in range(current_year, current_year + 2):
            ...
            for festival in settings.FESTIVALS:
                if not include_disabled and not festival.get('enabled', True):
                    continue
                ...
                try:
                    ...
                    match festival:
                        case {'type': 'fixed', 'month': month, 'day': day}:
                            target = datetime.date(year, month, day)
                        case {'type': 'lunar', 'month': month, 'day': day}:
                            target = LunarDate(year, month, day).to_solar_date()
                        case {'type': 'week', 'month': month, 'nth': nth, 'weekday': weekday}:
                            target = cls.get_nth_weekday(year, month, nth, weekday)
                        case {'type': 'term', 'nth': nth, 'term_name': term}:
                    ...
                    log.debug(f"Skipping festival {festival.get('name')} ({year}): {e}")
    ...
    def get_year_progress(current_time: datetime.datetime | None = None) -> float:
        ...
        next_year_start = datetime.datetime(target_year + 1, 1, 1).replace(tzinfo=now.tzinfo)
